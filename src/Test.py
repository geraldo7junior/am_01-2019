...
def main():
    ...
    data_class = np.loadtxt('../saidas/Execução-29-crisp-partition.txt', dtype=int)
...
def best_J():
    ...
    indices = []
    for i in range(100):
        teste = np.loadtxt('../saidas/Execução-{}-crisp-partition.txt'.format(i+1), dtype=int)
        if len(set(teste)) == 10:
            count += 1
            indices.append(i)
    print(count)
    ...
    jotas = []
    for i in indices:
        file = open('../saidas/Execução-{}-output.txt'.format(i+1), "r")
